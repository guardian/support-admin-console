...
import BannerTestsController._
...
object BannerTestsController {
  def fastlyPurger(stage: String, ws: WSClient)(implicit ec: ExecutionContext): Option[FastlyPurger] = {
    stage match {
      case "PROD" => Some(new FastlyPurger("https://support.theguardian.com/banner-tests.json", ws))
      case "CODE" => Some(new FastlyPurger("https://support.code.dev-theguardian.com/banner-tests.json", ws))
      case _ => None
    }
  }

  // For extracting the banner test name from an S3 key
  private val testNamePattern = """^.*/([\w-].*)\.json$""".r

  def extractTestName(key: String): Option[String] = key match {
    case BannerTestsController.testNamePattern(name) => Some(name)
    case _ => None
  }

  def archiveObjectSettings(stage: String, fileName: String) = S3ObjectSettings(
    bucket = "support-admin-console",
    key = s"$stage/archived-banner-tests/$fileName",
    publicRead = false
  )
}
...
class BannerTestsController(
  ...
  runtime: DefaultRuntime
    ...
    dataObjectSettings = S3ObjectSettings(
      ...
      key = s"banner/$stage/banner-tests.json",
    ...
    fastlyPurger = BannerTestsController.fastlyPurger(stage, ws),
    runtime = runtime
  ) with Circe {
  ...
  /**
    ...
    */
  def archive = authAction.async(circe.json[BannerTest]) { request =>
  ...
  def getArchivedTest(testName: String) = authAction.async { request =>
    ...
    run {
      S3Json
        .getFromJson[BannerTest](s3Client)
        .apply(objectSettings)
